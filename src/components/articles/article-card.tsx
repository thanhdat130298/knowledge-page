...
export function ArticleCard({ article }: { article: Article }) {
  return (
    <article className="surface-card group flex h-full flex-col overflow-hidden transition hover:-translate-y-0.5">
      <Link href={`/articles/${article.slug}`} className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto flex flex-wrap items-center gap-3 pt-4 text-xs text-muted">
          ...
          <span className="inline-flex items-center gap-1">
            <Star className="h-3.5 w-3.5" /> {article.rating_avg?.toFixed(1) ?? "—"}
          </span>
          <span className="inline-flex items-center gap-1">
            <MessageCircle className="h-3.5 w-3.5" /> {article.comment_count ?? 0}
          </span>
          <span className="inline-flex items-center gap-1">
            <Bookmark className="h-3.5 w-3.5" /> {article.bookmark_count ?? 0}
          </span>
        </div>
        {article.tags?.length ? (
